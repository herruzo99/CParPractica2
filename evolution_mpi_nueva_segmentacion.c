...
// Easy in-lining
#define accessMat2(arr, position) arr[position]
 ...
 */
	...
	double timeCellSyncT = 0.0f;		 // Sync
	...
	*/
	...
	int self_positions = (rows * columns) / nprocs; // First, distribute the position between the processes
	int offset_positions = (rows * columns) % nprocs; // Second, calculate the positions of the matrix that are left
	// Third, distribute the positions left between the processes
	int max_proc_position[nprocs];
	for ( i=0; i<nprocs; i++){
		// The max position of the process will be the sum of the positions that belong to other process
		// + his own positions + an extra position if there is something left (this positions are distributed on the first processes)
		max_proc_position[i] = (i+1) * self_positions + (i < offset_positions ? i + 1 : offset_positions);
	}
	int min_proc_position = rank * self_positions + (rank < offset_positions ? rank : offset_positions);
	...
	*/
	...
	for (i = 0; i < num_cells; i++)
	{
		...
		// 	num_init_cells++;
		// }
		// Check if the cell is in the positions that belong to the process
		int cell_position = (int)cells[i].pos_row * columns + (int)cells[i].pos_col; // Position of the cell in the matrix
		if (min_proc_position <= cell_position && cell_position < max_proc_position[rank])
		{
			cells[num_init_cells] = cells[i];
			num_init_cells++;
		}
	...
	for (iter = 0; iter < max_iter && current_max_food <= max_food && any_cell_alive; iter++)
	{
		...
		while (i + step_dead_cells < num_cells)
		{
			...
			for (j = nprocs - 1; j >= 1; j--)
			{
				...
				int cell_position = (int)cells[i].pos_row * columns + (int)cells[i].pos_col; // Position of the cell in the matrix
				if (cell_position < max_proc_position[j - 1])
				{
					//printf("Rank: %d Cell %d (%lf) lower than %d\n", rank, i, cells[i].pos_row, j);
					// EXPLICACION NECESARIA
					cell_exchange_aux = cells[cells_to_exchange[j]];
				...
				else if (cell_position < max_proc_position[j])
				{
					//			printf("Rank: %d Cell %d (%lf) in %d\n", rank, i, cells[i].pos_row, j);
					// EXPLICACION NECESARIA
					cells_to_exchange[j]++;
			...
			if (still_in_this_proc)
			{
				...
				// EXPLICACION NECESARIA
				int cell_position = (int) total_cells_recived[num_cells_in_this_proc].pos_row * columns + (int) total_cells_recived[num_cells_in_this_proc].pos_col;
				accessMat2(culture_cells, cell_position - min_proc_position) += 1;
				...
				//EXPLICACION NECESARIA
				food_to_share[num_cells_in_this_proc] = accessMat2(culture, cell_position - min_proc_position);
		...
		*/
		...
		for (i = 0; i < nprocs; i++)
		{
			...
			for (j = 0; j < num_cells_to_recive[i]; j++)
			{
				...
				int cell_position = (int) cells_recived[j].pos_row * columns + (int) cells_recived[j].pos_col;
				accessMat2(culture_cells, cell_position - min_proc_position) += 1;
				/* 4.3.5. Annotate the amount of food to be shared in this culture position */
				//food_to_share[num_cells_in_this_proc + j] = accessMat(culture, cells_recived[j].pos_row - min_proc_rows, cells_recived[j].pos_col);
				food_to_share[num_cells_in_this_proc + j] = accessMat2(culture, cell_position - min_proc_position);
			}
...
/* 4.4. Cell actions */
		...
		for (i = 0; i < num_cells; i++)
		{
			int cell_position = (int)cells[i].pos_row * columns + cells[i].pos_col;
			...
			short count = accessMat2(culture_cells, cell_position - min_proc_position);
			...
			accessMat2(culture, cell_position - min_proc_position);
		} // End cell actions
#if !defined(CP_TABLON)
...
/* 4.8. Decrease non-harvested food */
		...
		for ( i=0; i<self_positions; i++){
			accessMat2(culture_cells, i) = 0.0f;
			accessMat2(culture, i) = 0.95f; //Reduce 5%
			if (accessMat2(culture, i) > current_max_food)
				current_max_food = accessMat2(culture, i);
		}
...
#endif
		int aux = 0, relative_num_cells_sum = 0, cells_per_proc;
		...
		/* EN PROCESO particionado de la matriz dinámicamente

		cells_per_proc = aux / nprocs;
		...
		for (i = 0; i < nprocs;i++)			printf("Rank: %d num_cells: %d Fir %d max_proc_act: %d new_max_proc: %d \n", rank, num_cells,i, max_proc_rows[i], new_proc_rows[i]);
		...
		culture = (float *)realloc(culture, sizeof(float) * (size_t)self_rows * columns);
		culture_cells = (short *)realloc(culture_cells, sizeof(short) * (size_t)self_rows * columns);
		if (culture == NULL || culture_cells == NULL)
		{
			fprintf(stderr, "-- Error allocating culture structures for size: %d x %d \n", self_rows, columns);
			MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
