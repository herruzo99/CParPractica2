...
/* 4. Simulation */
	...
	for (iter = 0; iter < max_iter && current_max_food <= max_food && any_cell_alive; iter++)
	{
		...
		if (iter % syncCulture == 0)
		{
			...
			sndcnts[rank] = 0;
			displs[rank] = 0;
			printf("Iter: %d Rank: %d  for me dis %d cnt %d\n", iter, rank, my_dis, my_cnt);
			...
			for (i = 0; i < nprocs; i++)
			{
				printf("Iter: %d Rank: %d to %d dis %d cnt %d\n", iter, rank, i, displs[i], sndcnts[i]);
			}
